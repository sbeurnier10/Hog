...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score > 0:
        #print(opponent_score,player_score)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    """
    # BEGIN PROBLEM 6
    difference = score0 - score1
    if difference > 0 and last_leader == None: 
        return 0, f"Player 0 takes the lead by {difference}"
    elif difference < 0 and last_leader == None:
        return 1, f"Player 1 takes the lead by {abs(difference)}" 
    elif difference == 0:
        return None, None
    elif difference > 0 and last_leader == 1:
        return 0, f"Player 0 takes the lead by {difference}"
    elif difference < 0 and last_leader == 0:
        return 1, f"Player 1 takes the lead by {abs(difference)}"
    else:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i = 1
    while i <= 10:
        averaged_score = make_averaged(roll_dice, total_samples)(i, dice)
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_hogs_points = hefty_hogs(score, opponent_score)
    if hefty_hogs_points >= threshold or hog_pile((score + hefty_hogs_points), opponent_score) != 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
